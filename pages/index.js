import Head from "next/head";
import Image from "next/image";
import Link from "next/link";

export default function Home() {
  return (
    <div>
      <Head>
        <title>Kyle Mayer</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <header>
        <nav className="px-5 py-4  sm:flex text-gray-100">
          <div className="mr-12 mb-2 text-gray-100">Kyle Mayer</div>
          <div className="flex space-x-4 text-gray-100">
            <div>Home</div>
            <div>My Work</div>
            <div>About Me</div>
            <div>Contact</div>
          </div>
        </nav>

        <div className="pt-5 px-7 sm:container md:container mx-auto">
          {/* <Image
            src="/unnamed.png"
            className="rounded-full"
            alt="creature"
            height={100}
            width={100}
          /> */}
          <div className="text-gray-100 font-bold">Hello</div>

          <div className="text-gray-100">about me blurb</div>
        </div>
      </header>

      <body>
        <div className="pt-5 px-7 sm:container md:container mx-auto">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {/* Project 1 */}
            <div className="border-black shadow-sm hover:shadow-lg rounded-xl bg-gradient-to-r from-gray-800  via-gray-800  to-gray-900">
              <div className="text-gray-100 text-bold text-lg px-4 pt-5">
                New Growth PDX
              </div>
              {/* <div className="grid place-items-center">
                <Image
                  src={"/voz_visions.png"}
                  alt="test"
                  height={215}
                  width={340}
                />
              </div> */}
              <div className="text-gray-200 text-sm px-4 py-3">
                Web application with Google Maps integration enabled users to
                view recent local bird sightings. Icons are rendered to a map
              </div>
              <div className="flex px-3 py-3 items-center">
                <button className="bg-gray-900 hover:bg-blue-900 transition duration=250 rounded-full text-gray-100 text-sm px-4 py-2">
                  Details
                </button>
                <div className="ml-6 flex space-x-1  items-center">
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    React/Node
                  </div>
                  <div className="border border-gray-600  rounded-full text-white text-xs px-3 py-1">
                    Tailwind
                  </div>
                </div>
              </div>
            </div>
            {/* Project 2 */}
            <div className="border-black shadow-sm hover:shadow-lg  rounded-xl bg-gradient-to-r from-gray-800  via-gray-800  to-gray-900">
              <div className="text-gray-100 text-bold text-lg px-4 pt-5">
                Magic Link Login
              </div>
              <div className="text-gray-200 text-sm px-4 py-3">
                Web application with Google Maps integration enabled users to
                view recent local bird sightings. Icons are rendered to a map
              </div>
              <div className="flex flex-cols-2 px-3 py-3 items-center">
                <button className="bg-gray-900 hover:bg-blue-900 transition duration=250 rounded-full text-gray-100 text-sm px-4 py-2">
                  Details
                </button>
                <div className="ml-6 flex space-x-1">
                  <div className="border border-gray-600  rounded-full text-gray-200 text-xs px-3 py-1 ">
                    NextJS
                  </div>
                  <div className="border border-gray-600  rounded-full text-gray-200 text-xs px-3 py-1 ">
                    Magic Link
                  </div>
                </div>
              </div>
            </div>
            {/* Project 3 */}
            <div className="border-black shadow-sm hover:shadow-lg  rounded-xl bg-gradient-to-r from-gray-800  via-gray-800  to-gray-900">
              <div className="text-gray-100 text-bold text-lg px-4 pt-5">
                Birdzai Bird App
              </div>
              <div className="text-gray-200 text-sm px-4 py-3">
                Web application with Google Maps integration enabled users to
                view recent local bird sightings. Icons are rendered to a map
              </div>
              <div className="flex flex-cols-2 px-3 py-3 items-center">
                <button className="bg-gray-900 hover:bg-blue-900 transition duration=250 rounded-full text-gray-100 text-sm px-4 py-2">
                  Details
                </button>
                <div className="ml-6 flex space-x-1">
                  <div className="border border-gray-600  rounded-full text-white text-xs px-3 py-1 ">
                    React
                  </div>
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    Node
                  </div>
                </div>
              </div>
            </div>
            {/* Project 4 */}
            <div className="border-black shadow-sm hover:shadow-lg rounded-xl bg-gradient-to-r from-gray-800  via-gray-800  to-gray-900">
              <div className="text-gray-100 text-bold text-lg px-4 pt-5">
                Brewster Coffee API
              </div>
              <div className="text-gray-200 text-sm px-4 py-3">
                Web application with Google Maps integration enabled users to
                view recent local bird sightings. Icons are rendered to a map
              </div>
              <div className="flex flex-cols-2 px-3 py-3 items-center">
                <button className="bg-gray-900 hover:bg-blue-900 transition duration=250 rounded-full text-gray-100 text-sm px-4 py-2">
                  Details
                </button>
                <div className="ml-6 flex space-x-1">
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    React/Node
                  </div>
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    Tailwind
                  </div>
                </div>
              </div>
            </div>
            {/* Project 5 */}
            <div className="border-black shadow-sm hover:shadow-lg rounded-xl bg-gradient-to-r from-gray-800  via-gray-800  to-gray-900">
              <div className="text-gray-100 text-bold text-lg px-4 pt-5">
                Supabase Auth
              </div>
              <div className="text-gray-200 text-sm px-4 py-3">
                Web application with Google Maps integration enabled users to
                view recent local bird sightings. Icons are rendered to a map
              </div>
              <div className="flex flex-cols-2 px-3 py-3 items-center">
                <button className="bg-gray-900 hover:bg-blue-900 transition duration=250 rounded-full text-gray-100 text-sm px-4 py-2">
                  Details
                </button>
                <div className="ml-6 flex space-x-1">
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    NextJS
                  </div>
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    Supabase
                  </div>
                </div>
              </div>
            </div>
            {/* Project 6 */}
            <div className="border-black shadow-sm hover:shadow-lg rounded-xl bg-gradient-to-r from-gray-800  via-gray-800  to-gray-900">
              <div className="text-gray-100 text-bold text-lg px-4 pt-5">
                Raincoast Candles
              </div>
              <div className="text-gray-200 text-sm px-4 py-3">
                Web application with Google Maps integration enabled users to
                view recent local bird sightings. Icons are rendered to a map
              </div>
              <div className="flex flex-cols-2 px-3 py-3 items-center">
                <button className="bg-gray-900 hover:bg-blue-900 transition duration=250 rounded-full text-gray-100 text-sm px-4 py-2">
                  Details
                </button>
                <div className="ml-6 flex space-x-1">
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    NextJS
                  </div>
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    Shopify
                  </div>
                </div>
              </div>
            </div>
            {/* Project 7 */}
            <div className="border-black shadow-sm hover:shadow-lg rounded-xl bg-gradient-to-r from-gray-800  via-gray-800  to-gray-900">
              <div className="text-gray-100 text-bold text-lg px-4 pt-5">
                Obsidian Gardens
              </div>
              <div className="text-gray-200 text-sm px-4 py-3">
                Web application with Google Maps integration enabled users to
                view recent local bird sightings. Icons are rendered to a map
              </div>
              <div className="flex flex-cols-2 px-3 py-3 items-center">
                <button className="bg-gray-900 hover:bg-blue-900 transition duration=250 rounded-full text-gray-100 text-sm px-4 py-2">
                  Details
                </button>
                <div className="ml-6 flex space-x-1">
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    React
                  </div>
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    REST API
                  </div>
                </div>
              </div>
            </div>
            {/* Project 8 */}
            <div className="border-black shadow-sm hover:shadow-lg rounded-xl bg-gradient-to-r from-gray-800  via-gray-800  to-gray-900">
              <div className="text-gray-100 text-bold text-lg px-4 pt-5">
                Cabin Beer
              </div>
              <div className="text-gray-200 text-sm px-4 py-3">
                Web application with Google Maps integration enabled users to
                view recent local bird sightings. Icons are rendered to a map
              </div>
              <div className="flex flex-cols-2 px-3 py-3 items-center">
                <button className="bg-gray-900 hover:bg-blue-900 transition duration=250 rounded-full text-gray-100 text-sm px-4 py-2">
                  Details
                </button>
                <div className="ml-6 flex space-x-1">
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    React
                  </div>
                  <div className="border border-gray-600 rounded-full text-white text-xs px-3 py-1 ">
                    REST API
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </body>
    </div>
  );
}
